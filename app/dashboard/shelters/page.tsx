    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-foreground">Shelters</h1>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative flex-1 max-w-sm">
            ...
            <input
              ...
              className="w-full border border-border bg-background py-2 pl-9 pr-4 text-sm rounded-md placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary"
            />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center border border-border border-dashed rounded-lg p-12 text-center bg-card">
            <Home className="h-8 w-8 text-muted-foreground mb-3" />
            <h3 className="font-semibold text-foreground text-base">No Shelters Found</h3>
              ...
              <table className="w-full text-left border-collapse text-sm">
                ...
                <tbody className="divide-y divide-border">
                  {filteredShelters.map((shelter) => {
                    ...
                    return (
                      <tr key={shelter.id} className="hover:bg-muted/5 transition-colors">
                        <td className="p-4">
                        ...
                        <td className="p-4">
                          <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-semibold ${
                            ...
                            <span className={`h-1.5 w-1.5 rounded-full ${isFull ? "bg-destructive" : "bg-success"}`} />
                            {isFull ? "Full" : "Open"}
                          </span>
                        </td>
                        <td className="p-4">
                            ...
                            <div className="h-1.5 w-full bg-secondary rounded-full overflow-hidden">
                              <div 
                                className={`h-full rounded-full transition-all ${
                                  occupancyRate >= 90 ? "bg-destructive" : occupancyRate >= 70 ? "bg-warning" : "bg-primary"
                                }`} 
